...
SixtyMins.Controller.PreviousSegmentsGrid = function() {
	...
	callbacks = {
		...
		populate : function(_segments) {
			...
			SixtyMins.Lookup.getView('PreviousSegments').hidePreloader();
	...
	this.showCategory = function(id){
		...
		if(id != currentId)
		{
			...
			// special case for first tab - "recent"
			if(id == 0){
			  id = SixtyMins.Lookup.getModel('Base').getCategoriesIds().join(",");      
			}			
...
SixtyMins.Controller.PreviousSegmentsGrid.prototype = new SixtyMins.Controller.Base();
