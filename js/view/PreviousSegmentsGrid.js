...
SixtyMins.View.PreviousSegmentsGrid = function(name) {
	...
	var $wrapper 	 = this.domElement,
		...
		data	 	 = null,
		dataDrawn	 = null,
		rootURL      =  '/htdocs/60minutesapp/previous-segments/',
		showTimeouts = []
	;
	...
	this.draw = function() {
		...
		if(data && data.length){
			...
			for(var i = 0; i < data.length; i++){
                ...
                showTimeouts.push(setTimeout(callback, 200, $elem, i/10));
			}
			// add listeners
		...
		$wrapper.bind('scroll', function(){
			...
			eventThrottle.run(function(){
				...
				if((parseInt($list.height()) - parseInt($wrapper.scrollTop())) <= parseInt($wrapper.height())){
					SixtyMins.Lookup.getView('PreviousSegments').showPreloader();
					var model = SixtyMins.Lookup.getModel('PreviousSegmentsGrid'),
						controller = SixtyMins.Lookup.getController('PreviousSegmentsGrid');
					// Run AJAX Query
	...
	this.clearGrid = function() {
		...
		while(showTimeouts.length)
		{
			clearTimeout(showTimeouts.shift());
		}
	};
	...
	var callbacks = {
            ...
            onBackClick: function(e) {
                 ...
                 SixtyMins.RouteManager.pushState(null,null, rootURL + SixtyMins.Lookup.getView('PreviousSegments').openedTab);
...
//        console.log(this, 'show video:', data, typeof me.getDataSetForID, me);
        ...
        if(SixtyMins.Lookup.getController('PreviousSegments').data != undefined){
            if(SixtyMins.Lookup.getController('PreviousSegments').data.firstState == false){
                SixtyMins.RouteManager.pushState(null,null, rootURL + SixtyMins.Lookup.getView('PreviousSegments').openedTab + '/video/' + $thisElement.find('figure').data('id'));
            }
        } else {
            SixtyMins.RouteManager.pushState(null,null, rootURL + SixtyMins.Lookup.getView('PreviousSegments').openedTab + '/video/' + $thisElement.find('figure').data('id'));
        }
...
//    	console.log(this,'getDataSetForID', $list, $element);
...
SixtyMins.View.PreviousSegmentsGrid.prototype = new SixtyMins.View.Base();